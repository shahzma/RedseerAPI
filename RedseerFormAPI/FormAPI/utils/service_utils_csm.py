...
class CalculatedParamCSMFn:
    ...
    def WA_Calc(self,pl,sd,ed,rep_ver_id,db):
        try:
            ...
            df=pd.read_excel(yls,"Content and SM", header=None, index_col=False)
            required_df=[]
            ...
            print(f'{datetime.now().strftime("[%d/%b/%Y %H:%M:%S]")}  Error:WA Error :-',e)

    
    def calc_script_CSM(self, pl_id, sd, ed,pr_sd,pr_ed,rep_ver_id,db):
        try:
            calc_dict = {}
            print(pl_id)
            try:
            ...
            self.InsertORUpdate(required_df,db)
            print(f'{datetime.now().strftime("[%d/%b/%Y %H:%M:%S]")}  Log:Calc Script Ran successfully')
        except Exception as e:
    ...
    def report_version_id(self, rep_ver_id, callback):
        try:
            ...
            print(p_sd)
            self.calc_script_CSM(pl,sd,ed,str(p_sd),str(p_ed),rep_ver_id,db)
            self.WA_Calc(pl,sd,ed,rep_ver_id,db)
            cur.close()
            db.close()
            print("CSM Script:- report_version_id finished")
            callback()
        except Exception as e:
            print(print(f'{datetime.now().strftime("[%d/%b/%Y %H:%M:%S]")}  Error:Report Version Id :-',e),e)
